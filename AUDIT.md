...
| 2026-05-21 | M2 | _pending_ | Login form now reads `?email=` from the search params, sanitises (length≤254, must contain '@') and uses it as the RHF default. autoFocus moves to the password field when prefilled so the user can type their password immediately. Closes the loop on register-{artist,caster} which already redirect to `/login?email=…` on the duplicate-email path. |
