...
| 2026-05-21 | M18 | _pending_ | step-identity now uses `react-dropzone` (single-file, mime-restricted) like step-portfolio. Drag-and-drop, drag-active styling, and the "Replace" button is rewired to `dropzone.open()` so click-to-browse still works. Hidden manual `<input ref={inputRef}>` removed. |
