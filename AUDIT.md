...
| 2026-05-21 | M4 | _pending_ | `fetcher` now skips its `window.location.href = '/login'` redirect when the user is already on `/login`, `/register*`, `/forgot-password`, `/reset-password*`, `/verify-email*`, or `/suspended`. Stops hard-reloads from clobbering an inline error state (e.g. wrong-password attempt on /login) and removes the redirect-loop risk on auth pages that issue background v1 requests. |
