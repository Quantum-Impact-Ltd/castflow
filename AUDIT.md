...
| 2026-05-21 | M22 | _pending_ | Pending-page "Check application status" button now enforces a 30s cooldown after each click. The button label switches to a live `Check again in Ns` countdown driven by a 1Hz interval that only ticks while the cooldown is active (no idle re-renders once it expires). |
