...
export function VerifyTokenClient({ token }: VerifyTokenClientProps) {
  ...
  // Verify automatically on mount — the user already expressed intent by
  // clicking the link in their email, so a second "Confirm" click was just
  // friction. The ref guards against React StrictMode's double-invoke (and a
  // one-time token would 'error' on the second call). (Audit #6.)
  ...
  useEffect(() => {
    ...
    verifyEmailToken(token)
      ...
      .catch((err) => {
        // One generic message (don't leak expired-vs-unknown), but log it.
  ...
  // pending — verifying automatically
  return (
    <div className="flex flex-col items-center gap-5 text-center">
      <span className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-[var(--cta-400)]/15 text-[var(--cta-400)]">
