...
export function BookingDetailClient({ id }: { id: string }) {
  ...
  const fullySigned = b.contract?.status === 'fully_signed'
        ...
        actions={
          <div className="flex gap-2">
            <Button asChild variant="outline">
              <Link href={`/artist/bookings/${b.id}/contract`}>Contract</Link>
            </Button>
            {b.status === 'completed' ? (
              <Button asChild variant="outline">
                <Link href={`/artist/bookings/${b.id}/review`}>Leave review</Link>
              </Button>
            ) : null}
            {b.status === 'completed' || b.status === 'confirmed' ? (
              <Button asChild variant="outline">
                <Link href={`/artist/bookings/${b.id}/dispute`}>Raise dispute</Link>
              </Button>
            ) : null}
            {canCancel ? (
              <Dialog open={open} onOpenChange={setOpen}>
                ...
                <DialogContent>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Cancellations within 48h of the shoot incur a fee — see the cancellation
                      policy.
                    </DialogDescription>
                  </DialogHeader>
                  <Textarea
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="grid grid-cols-2 gap-4 text-sm">
            ...
            <Field
              label="Call time"
              value={b.callTime ? b.callTime : 'Revealed when contract signed'}
            />
            <Field
              label="Location"
              value={fullySigned ? b.shootLocation : 'Revealed when contract signed'}
