...
export function ReviewClient({ bookingId }: { bookingId: string }) {
  ...
  const existingMine = (reviews.data ?? []).find(
    (r) => r.reviewerRole === 'caster' && (!myProfileId || r.reviewerId === myProfileId),
  )
  ...
  const artistName = booking.data.artist?.firstName ?? 'the artist'
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        title="Rate the artist"
        description={booking.data.job?.title ?? 'CastFlow shoot'}
      />
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                rows={5}
                maxLength={500}
                placeholder={`Share how the shoot went with ${artistName} — professionalism, punctuality, performance…`}
